   ...
   stage("Quality gate"){
        ...
        // to trigger other pipeline to update jenkins manifest
         stage("Trigger CD Pipeline") {
            steps {
                script {
                    sh "curl -v -k --user raemond:${JENKINS_API_TOKEN} -X POST -H 'cache-control: no-cache' -H 'content-type: application/x-www-form-urlencoded' --data 'IMAGE_TAG=${IMAGE_TAG}' 'http://192.168.100.74:8080/job/gitops-complete-prodcution-e2e-pipeline/buildWithParameters?token=gitops-token-e2e'"
                }
            }
        }
    }
